...
$createRevStatsBody = "{`"charge_type_ids`":[`"$($($chargeTypes | Where-Object { $_.code -eq "ENSRR" }).id)`",`"$($($chargeTypes | Where-Object { $_.code -eq "RR" }).id)`",`"$($($chargeTypes | Where-Object { $_.code -eq "RRA" }).id)`",`"$($($chargeTypes | Where-Object { $_.code -eq "NSRR" }).id)`",`"$($($chargeTypes | Where-Object { $_.code -eq "PEC" }).id)`",`"$($($chargeTypes | Where-Object { $_.code -eq "HHRR" }).id)`",`"$($($chargeTypes | Where-Object { $_.code -eq "MRR" }).id)`",`"$($($chargeTypes | Where-Object { $_.code -eq "RTC" }).id)`",`"$($($chargeTypes | Where-Object { $_.code -eq "ADRR" }).id)`", `"$($($chargeTypes | Where-Object { $_.code -eq "NSCF" }).id)`"],`"other_room_revenue_charge_type_ids`":[],`"payment_type_ids`":[],`"calculation_type`":`"REVENUE`",`"exclude_adjustments`":false,`"id`":null,`"created_at`":null,`"updated_at`":null,`"deleted_at`":null}"
